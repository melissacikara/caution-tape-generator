...
serve(async (req) => {
  ...
  if (existing.owner_id) {
    if (!sameUuid(userId, existing.owner_id as string)) {
  ...
  const decision = await orphanClaimDecision(supabase, existing.id as string, userId)
  if (decision === 'forbidden') {
    return jsonError(req, 'FORBIDDEN', 'You do not have permission to modify this scenario', 403)
  }
  if (decision === 'unattributed') {
    return jsonError(req, 
      'NO_OWNER',
      'This scenario has no owner and the tapes are all anonymous. Create a new scenario while signed in, or ask an admin to set owner_id in the database.',
      409,
    )
  }
  ...
  const { data: rows, error: uErr } = await supabase
    .from('scenarios')
    .update({ is_public: isPublic, owner_id: userId })
    .eq('id', existing.id)
    .select('id, name, public_slug, owner_id, is_public, created_at, updated_at')
